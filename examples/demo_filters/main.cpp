...
int main(){
    ...
    niaFrameBuffer frameBuffer(1200, 728, GL_COLOR_ATTACHMENT0);
    niaShaderQuad quadShader;
    niaPostProcessingPipeline pipeline(1200, 728, frameBuffer);

    pipeline.addFilter(new niaFilterInvert());
    pipeline.addFilter(new niaFilterBoxBlur());
    ...
    while(!window.isClosed()){
        ...
        pipeline.processPipeline(&renderer);

        quadShader.useShader();
        renderer.renderMeshRaw(niaMesh::quad(1.0), pipeline.getFrameBufferTextureId());
