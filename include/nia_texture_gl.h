...
public:
    GLuint textureId;
    niaTexture();
    niaTexture(const char* filename, u32 flag = NIA_TEXTURE_NO_FLAG, u16 frameNumber = 0);
    niaTexture(const char* filename);
    niaTexture(u8* data, u32 width, u32 height, const textureFormatDetails& details);
    ~niaTexture();
